	...
	public class FlutterSlime : ModNPC
	{
		...
		public override void AI()
		{
			...
			if (AI_State == State_Asleep)
			{
				...
				if (npc.HasValidTarget && Main.player[npc.target].Distance(npc.Center) < 100000f)
				{
			...
			else if (AI_State == State_Notice)
			{
				...
				if (Main.player[npc.target].Distance(npc.Center) < 100000f)
				{
			...
			else if (AI_State == State_Hover)
			{
				...
				if (AI_Timer == 1 && Main.netMode != NetmodeID.MultiplayerClient)
				{
					...
					AI_FlutterTime = Main.rand.NextBool() ? 200 : 100;
					npc.netUpdate = true;
				}
